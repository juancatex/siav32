<template>
            ...
            <!-- Breadcrumb -->
            <ol class="breadcrumb">
                <li class="breadcrumb-nomtipodevolucion"><a href="/">Liq. Saldos Menores </a></li>
            </ol>
            <div class="container-fluid">
                <!-- Ejemplo de tabla Listado -->
                <div class="card">
                    <div class="card-header">
                        <i class="fa fa-align-justify"></i>Liq. Saldos Menores                        
                    </div>
                    <div class="card-body">
                        <div class="form-group row">
                            <div class="col-md-6">
                                <div class="input-group">                                                            
                                    <button type="submit" @click="abrirModal('liquidarsaldos','registrar')" class="btn btn-primary"><i class="fa fa-search"></i>Procesar</button>
                                </div>
                            </div>
                        </div>                        
                    </div>
                ...
                <table class="table table-bordered table-striped table-sm">
                    ...
                    <tbody>
                        <tr v-for="procesados in arrayProcesados" :key="procesados.id">
                            <td>
                                <button type="button" @click="reporte_liq(procesados.fec_liquidacion,dev_liqsaldo)" class="btn btn-warning btn-sm" title="Reportes">
                                    <i class="icon-book-open"></i>
                                </button>&nbsp;
                            </td>
                            <td>{{procesados.fec_liquidacion}}</td>
                ...
                <div class="modal-dialog modal-primary modal-lg" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-md-3">
                                        <input v-validate.initial="'required'" 
                                                ...
                                                name="saldomenor">
                                    <span class="text-error">{{ errors.first('saldomenor')}}</span>
                                    </div>                                    
                                </div>
                                <div class="form-group row">
                            ...
                            <tbody>
                                <tr v-for="saldos in arraySaldosmenores" :key="saldos.id">                                                                        
                                    <td v-text="saldos.codproducto"></td>
                                    <td v-text="saldos.no_prestamo"></td>
                                    <td v-text="saldos.numpapeleta"></td>
                                    <td v-text="saldos.nombre"></td>
                                    <td v-text="saldos.cut"></td>
                        ...
                        <div class="modal-footer">
                            ...
                            <button  v-if="tipoAccion==2 && arraySaldosmenores!=''" :disabled = "errors.any()" type="button" class="btn btn-warning" @click="liquidaresaldos()">Liquidar</button>
                            <button :disabled = "errors.any()" type="button" class="btn btn-primary" @click="procesarsaldos(fechaproceso, saldomenor)">Procesar</button>
...
<script>   
    ...
    export default {
        ...
        methods : {
            ...
            reporte_pro(id,liq_procesado){
              var url=liq_procesado + id;              
              repo.viewPDF(url,'Liquidacion procesada');
            },
            ...
            procesarsaldos(fechasaldos, saldomenor) {
                let me=this;
                var url= '/liq_saldosmenores/procesarsaldos?fechasaldos='+ fechasaldos + '&saldomenor='+ saldomenor;
                axios.get(url).then(function (response) {
                    var respuesta= response.data;
                    me.arraySaldosmenores = respuesta.saldosmenores; console.log(me.arraySaldosmenores);
                    me.tipoAccion=2;
                }). then(function (response) { 
                    if (me.arraySaldosmenores.length===0) {
            ...
            reporte_liq(fecha,dev_liqsaldo){
              var url=dev_liqsaldo + fecha;                console.log(url);
              repo.viewPDF(url,'Liq. Saldos Menores');
            }, 

            getRutasReports (){ 
                let me=this;
                var url= '/liq_saldosmenores/reportes';
                axios.get(url).then(function (response) {
                    var respuesta= response.data; ;
                    me.dev_liqsaldo=respuesta.LIQ_SALDOS_MENORES; 
            ...
            listarProcesados (page,buscar,criterio){
                let me=this;
                var url= '/liq_saldosmenores?page=' + page + '&buscar='+ buscar + '&criterio='+ criterio;
                axios.get(url).then(function (response) {
                    var respuesta= response.data;
                    me.arrayProcesados = respuesta.saldosprocesados.data; console.log(respuesta);
                ...
                switch(modelo){
                    case "liquidarsaldos":
                    {
                        switch(accion){
                            ...
                            {
                                this.modal = 1;
                                this.tituloModal='Proceso Saldo menores';
