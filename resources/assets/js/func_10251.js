    ...
    if (promises.montosolicitado > 0 && promises.plazomeses > 0 && !promises.errors.any()) {
      ...
      for (var idf in vector) {
        switch (parseInt(vector[idf].tipo)) {
          ...
          case 2:
              console.log('criterio caso 2 linea 383');
            console.log(vector[idf].value);
            break;
          case 3:
              console.log('criterio caso 3 linea 387');
            console.log(vector[idf].value);
            break;
        }
      }
...
export function _vm2154_12186_135(id,titulo) {
  ...
  return swal({ 
    ...
    '</label> <button id="close_id_swal" class="close" style="float: right; margin: 0 15px 2px 5px; ">X</button> </div>'+
    ...
    this.modales.set(id, {
      ...
      onclose: function () {
        $("#" + id).on('hidden.bs.modal', function () {
          console.log('se cerro el modal con id=' + id);
        });
      },
      openInfo: function () {
        $("#" + id).modal({
